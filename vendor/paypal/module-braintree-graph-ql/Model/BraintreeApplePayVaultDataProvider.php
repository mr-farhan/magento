...
class BraintreeApplePayVaultDataProvider implements AdditionalDataProviderInterface
{
    /**
    ...
    public function getData(array $data): array
    {
        if (!isset($data[ConfigProvider::METHOD_VAULT_CODE])) {
            throw new GraphQlInputException(
                __('Required parameter "braintree_applepay_vault" for "payment_method" is missing.')
            );
        }

        if (!isset($data[ConfigProvider::METHOD_VAULT_CODE]['public_hash'])) {
            throw new GraphQlInputException(
                __('Required parameter "public_hash" for "braintree_applepay_vault" is missing.')
            );
        }
